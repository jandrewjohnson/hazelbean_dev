...
class TestCOGCompliance(unittest.TestCase):
    ...
    def test_make_path_cog(self):
        """Test convert_to_cog_via_translate() Need to find a non-translate way. maybe rio?"""

        # START HERE: continue on the base data version of cogifying, then pogify.
        with self.subTest(file=self.invalid_cog_path):
            temp_path = hb.temp('.tif', remove_at_exit=True)
            convert_to_cog_via_translate(self.invalid_cog_path, temp_path, output_data_type=1, overview_resampling_method='mode', ndv=255, compression="ZSTD", blocksize=512, verbose=True)
